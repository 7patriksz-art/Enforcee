 ...
 * the list."
 *
 * A test cannot reach the project docs, but the CLI in `scripts/doc-claims.mjs` can be
 * pointed at them once a run has dumped them to disk:
 *
 *     npm run doc-claims -- /path/to/dumped/project-docs
 *
 * So the rules live HERE, in src/, the test imports them and CI runs them over the repo, and
 * the script bundles this module and runs the same rules over docs outside the tree. ONE
 * copy: E-1, the duplicated-source class, is at twelve instances on this project and a second
 * regex for these rules would be the thirteenth.
 *
 * WHY src/ AND NOT scripts/. The first version of this file was `scripts/doc-claims.mjs` with
 * a `scripts/doc-claims.d.mts` sidecar for types, imported directly by the test. It was green
 * on ubuntu and macos and threw `SyntaxError: Invalid or unexpected token` on windows-latest
 * at the import — the eighth Windows-only break on this project. It was the ONLY test
 * importing from `scripts/` and the ONLY `.d.mts` in the tree; every other test imports plain
 * TypeScript from `src/`, and every other test is green on Windows.
 * `tests/portability.test.ts` now enforces both halves of that.
 *
...
export function markdownFiles(root: string, dir = '.', out: string[] = []): string[] {
  let entries;
  try {
...
export function repoMechanisms(repoRoot: string): Mechanisms {
  ...
  return { scripts, commands, exists: (p: string) => existsSync(join(repoRoot, p)) };
}
...
export const isPlanDoc = (f: string): boolean => /(^|\/)PLAN-[A-Z-]+\.md$/.test(f);
 ...
 */
export function rules(docs: Map<string, string>, mech: Mechanisms): Results {
  const out: Results = {
      ...
      for (const line of body.split('\n')) {
        ...
        for (const m of line.matchAll(/`?(?:npx )?enforcee ([a-z][a-z-]*)/g)) {
          ...
          if (!mech.commands.has(m[1])) {
            out['plan-tick-off'].missing.push(
              `${f}: claims "enforcee ${m[1]}" is done — ${line.trim().slice(0, 96)}`
            );
          }
        }
        for (const m of line.matchAll(/`npm run ([a-z][\w:-]*)`/g)) {
          ...
          if (!mech.scripts[m[1]]) {
            out['plan-tick-off'].missing.push(
              `${f}: claims "npm run ${m[1]}" is done — ${line.trim().slice(0, 96)}`
            );
          }
        }
      }
