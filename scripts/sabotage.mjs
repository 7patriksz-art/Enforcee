...
const SABOTAGES = [
  ...
  // -- hearing it when it is said: the learning half of the loop -----------------
  {
    name: "said-capture-gate-drops-what-the-library-could-use",
    why: "the guard captures and the library decides. The gate is allowed to be crude ONLY because it is a superset - under-capture loses the sentence forever, and no later pass can recover a turn nobody wrote down",
    file: "src/lib/prevent/said.ts",
    find: "  if (t.length < 12 || t.length > 2000) return false;\n  return DIRECTIVE.test(t);",
    replace: "  if (t.length < 12 || t.length > 2000) return false;\n  return /\\bnever\\b/i.test(t);",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /the library can use these but the guard never kept them|the guard would have thrown this turn away/,
  },
  {
    name: "said-captures-every-turn-including-pasted-logs",
    why: "a pasted stack trace containing the word never is not somebody stating a preference, and capturing it buries the real ones",
    file: "src/lib/prevent/said.ts",
    find: "  if (t.length < 12 || t.length > 2000) return false;",
    replace: "  if (t.length < 12) return false;",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /a pasted log containing the word "never" was captured as a preference/,
  },
  {
    name: "said-counts-emphasis-as-evidence",
    why: "saying it three times in one breath is one preference. Counting it as three inflates the mention count that decides whether a rule gets offered",
    file: "src/lib/prevent/said.ts",
    find: "    if (seen.has(id)) continue;",
    replace: "    if (false) continue;",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /the same instruction was counted twice/,
  },
  {
    name: "said-store-grows-without-bound",
    why: "it lives in somebody repository and is appended to on a keystroke path",
    file: "src/lib/prevent/said.ts",
    find: "  return rows.length <= max ? rows : rows.slice(rows.length - max);",
    replace: "  return rows;",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /it dropped the newest instead of the oldest|toHaveLength/,
  },
  {
    name: "install-stops-wiring-the-learning-half",
    why: "UserPromptSubmit is the only event carrying the user own words. Unwired, learning goes back to being a thing a human must remember to run against a file",
    file: "src/lib/enforce/policy.ts",
    find: "      UserPromptSubmit: [{ matcher: '*', hooks: [{ type: 'command', command: cmd, timeout: 10 }] }],",
    replace: "",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /UserPromptSubmit is not wired, so the learning half never runs/,
  },
  {
    name: "learn-reports-an-empty-corpus-as-a-clean-answer",
    why: "\"no preferences found\" and \"no corpus was read\" look identical in a terminal and only one is a result. Six recorded instances of a scan silently covering nothing",
    file: "cli/dist/enforcee.mjs",
    find: "      if (rows.length === 0) {",
    replace: "      if (false) {",
    occurrences: 1,
    run: "tests/said.test.ts",
    expect: /an empty corpus was reported as a clean answer/,
  },
];
